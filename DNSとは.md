...
## ゾーン転送とは
1台のDNSサーバが停止してもサービス続行できるように、複数台のDNSサーバにゾーン情報を送信する機能。  
元のゾーン情報を保持するDNSサーバを**プライマリサーバ**、コピー先のDNSサーバを**セカンダリサーバ**という。  
特に制限をしなかった場合、セカンダリサーバ以外のDNSサーバからも転送要求ができるため、ゾーン情報を攻撃に悪用される可能性がある。  
そのため、プライマリDNSサーバにセカンダリサーバからの要求しか受け付けない設定を行うことが推奨される。
